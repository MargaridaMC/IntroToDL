...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        gammax = gamma * x_norm #scaling

        out = gammax + beta #shifting

        running_var = momentum * running_var + (1 - momentum) * var #The running average decays over time
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
...
def batchnorm_backward(dout, cache):
    ...
    dx_minus_mean2 = 2*dsq*x_minus_mean
    
    dmean = -np.sum(dx_minus_mean1 + dx_minus_mean2,axis=0)
