...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #layer1 = X.dot(W1) + b1 and apply ReLU
        layer1 = X.dot(W1) + b1
        rect_layer1 = np.maximum(layer1,0)

        scores = rect_layer1.dot(W2) + b2
        ...
        likelihood = exp_scores / np.sum(exp_scores, axis=1, keepdims=True) 
        log_likelihood = -np.log(likelihood[range(N),y])
        
        loss = np.sum(log_likelihood)/N + 0.5*reg*(np.sum(W1*W1) + np.sum(W2*W2))
        ...
        grads['W2'] = np.dot(rect_layer1.T,dscores) + reg*W2
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val, num_iters=1500, num_tests = 100,min_lr = 1e-6,max_lr = 1e-3, max_reg = 1000, min_reg = 0, min_hidden_size=500, max_hidden_size=500,min_batch_size=200,max_batch_size=200):
    ...
    num_classes = 10
    hidden_sizes = np.linspace(min_hidden_size,max_hidden_size,dtype=int)
    batch_sizes = np.linspace(min_batch_size,max_batch_size,dtype=int)
    learning_rates = np.linspace(min_lr,max_lr,num = 100000)
    regularization_strengths = np.linspace(min_reg,max_reg,num = 100000)
    ...
    for i in range(num_tests):
        ...
        print('lr %e reg %e hidden_size %d batch_size %d' % (lr, reg,hidden_size, batch_size))
