...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        #Apply ReLU
        rectified_layer1 = layer1
        rectified_layer1[layer1<0] = 0
        
        #scores = rectified_layer1.dot(W2) + b2
        scores = rectified_layer1.dot(W2) + b2
        ...
        likelihood = softmax(scores)
        log_likelihood = -np.log(likelihood[range(N),y])
        
        loss = np.sum(log_likelihood)/N + reg*np.sum(np.square(W1))/2 + reg*np.sum(np.square(W2))/2
        ...
        dscores = likelihood
        dscores[range(N),y] -= 1 #shape = num_inputs x num_classes
        dscores /= N
        
        grads['b2'] = np.sum(dscores,axis=0,keepdims=True)
        
        dW2 = np.dot(rectified_layer1.T,dscores)
        grads['W2'] = dW2 + reg*W2
        
        d_ReLU_output = np.dot(dscores,W2.T)
        d_ReLU_input = d_ReLU_output #shape = num_inputs x H
        d_ReLU_input[layer1<=0] = 0

        grads['b1'] = np.sum(d_ReLU_input,axis=0,keepdims=True)
        
        dW1 = np.dot(X.T,d_ReLU_input)
        grads['W1'] = dW1 + reg*W1
    ...
    def predict(self, X):
        ...
        probs = softmax(np.matmul(np.maximum(np.matmul(X,W1) + b1,0),W2) + b2)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val, num_iters=1500, num_tests = 100,batch_size=200,min_lr = 1e-6,max_lr = 1e-3, max_reg = 1000, min_reg = 0,hidden_size = 500):
    ...
    num_classes = 10#self.params['b2'].shape[0]
    #hidden_sizes = np.linspace(1,1000,dtype=int)
    
    learning_rates = np.linspace(min_lr,max_lr)
    #learning_rate_decay = np.linspace(0.5,1,num=10)
    regularization_strengths = np.linspace(min_reg,max_reg)
    #number_iterations = np.linspace(100,5000,dtype=int)
    
    best_val = -1
    best_net = None
    
    #Trying random selections of parameters
    for i in range(num_tests):
        print('Test number %d' %i)
        
        #hidden_size = np.random.choice(hidden_sizes,1)[0]
        ##num_iters = np.random.choice(number_iterations,1)[0]
    ...
    print('best net: lr %e reg %e hidden units %d' % (lr, reg, hidden_size))
    return best_net
